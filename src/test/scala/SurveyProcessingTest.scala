...
class SurveyProcessingTest extends FunSuite with DatasetSuiteBase {
...

  {
    test("Age average by Country") {
      val expectedAvg = Array(
        AgeAvgByCountry("Papua New Guinea", 63.0),
        AgeAvgByCountry("Saint Kitts and Nevis", 57.0)
      )

      val surveyDataFrame = readFromTestFile();
      val surveyProcessing: SurveyProcessing = new SurveyProcessing(surveyDataFrame, spark);

      val realAvg = surveyProcessing.createAgeAvgByCountry();
  ...
  {
    test("Percentage of programmers by languaje") {
      val expectedAvg = Array(
        PercentageProgrammersLanguageView("JavaScript", 59219, 13.403452545046726),
        PercentageProgrammersLanguageView("HTML/CSS", 55466, 12.554009673644636)
      )

      val surveyDataFrame = readFromTestFile();
      val surveyProcessing: SurveyProcessing = new SurveyProcessing(surveyDataFrame, spark);

      val realAvg = surveyProcessing.createPercentageProgrammersLanguage();
  ...
  {
    test("Percentage of programmers by platform.") {
      val expectedAvg = Array(
        PercentageProgrammersPlatformviews("Linux", 42753, 15.479729313834467),
        PercentageProgrammersPlatformviews("Windows", 40630, 14.71104722524956)
      )

      val surveyDataFrame = readFromTestFile();
      val surveyProcessing: SurveyProcessing = new SurveyProcessing(surveyDataFrame, spark);

      val realAvg = surveyProcessing.createPercentageProgrammersPlatform();
